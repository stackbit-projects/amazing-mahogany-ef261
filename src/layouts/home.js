              ...
              <div className="intro">
                <div className="inner-md">
                  ...
                  <div className="intro-text">
                    {markdownify(_.get(this.props, 'page.frontmatter.intro_content', null))}
                  </div>
                  )}
                  {_.get(this.props, 'page.frontmatter.intro_actions', null) && (
                  <div className="intro-cta">
                    {_.map(_.get(this.props, 'page.frontmatter.intro_actions', null), (action, action_idx) => (
                    <Link key={action_idx} href={withPrefix(_.get(action, 'url', null))} className={classNames({'button': (_.get(action, 'style', null) === 'primary') || (_.get(action, 'style', null) === 'secondary'), 'button-secondary': _.get(action, 'style', null) === 'secondary'})} {...(_.get(action, 'new_window', null) ? ({target: '_blank', rel: 'noopener'}) : null)}>{_.get(action, 'label', null)}</Link>
                    ))}
                  </div>
                  )}
                </div>
                    ...
                    <div className="post-card-content">
                      ...
                      <div className="post-excerpt">
                        ...
                        <p className="read-more">
                          <Link className="button button-secondary" href={withPrefix(_.get(post, '__metadata.urlPath', null))}>{_.get(this.props, 'page.frontmatter.more_link_text', null)}</Link>
                        </p>
                        )}
                      </div>
                    </div>
